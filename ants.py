...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while temp_place!=None:
            if not temp_place.is_hive and len(temp_place.bees)!=0 and self.min_range<=count<=self.max_range:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """A WallAnt that does nothing but has a large health value."""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """A HungryAnt that will select a random Bee from its place and eat it whole."""
    ...
    def action(self, gamestate):
        ...
        while temp_place!=None:
            if temp_place.ant!=None:
                Ant.buff(temp_place.ant)
                if temp_place.ant.is_container and temp_place.ant.ant_contained!=None:
                    Ant.buff(temp_place.ant.ant_contained)
